#! /usr/bin/env python2
# Run the slowest program and check that 
#  it is in fact slow enough to not finish in an hour.
...
class Row:
  def __init__(self, uuid, path, name, bitwidth, timeout, time_elapsed, exitcode, did_timeout):
    self.uuid = uuid
    self.path = path
    self.name = name
  ...
  def write_header(cls, csv_writer):
    csv_writer.writerow(["uuid", "path", "name", "bitwidth", "timeout", "time_elapsed", "exitcode", "did_timeout"])

  def write(self, csv_writer):
    csv_writer.writerow([self.uuid, self.path, self.name, self.bitwidth, self.timeout, self.time_elapsed, self.exitcode, self.did_timeout])


def verify_opt_with_timeout(uuid, path, opt, timeout, bitwidth):
  name, pre, src, tgt, ident_src, ident_tgt, used_src, used_tgt, skip_tgt = opt
  print '----------------------------------------'
  ...
  row = Row(uuid, path, name, bitwidth, timeout, end_time - start_time, exitcode, did_timeout)
  return row


def verify_all():
  out_path = "experiment-out-data/slowest.csv"
  ...
  slow_programs = {
          ...
          ("tests/instcombine/select.opt", "Select:746") : 1024,
          ("tests/instcombine/select.opt", "Select:747") : 1024,
          ("tests/instcombine/shift.opt","InstCombineShift: 239") : 1024,
          ("tests/instcombine/shift.opt","InstCombineShift: 279") : 1024,
          ("tests/instcombine/shift.opt","InstCombineShift: 440") : 1024,
          ("tests/instcombine/shift.opt","InstCombineShift: 476") : 1024,
          ("tests/instcombine/shift.opt","InstCombineShift: 497") : 1024,
          ("tests/instcombine/shift.opt","InstCombineShift: 582")  : 1024
   }
  ...
  for path in paths:
      with open(path, "r") as f:
        ...
        for opt in opts:
            name = opt[0]
            if (path, name) in slow_programs:
              print("found '%4d| %s %s'" % (len(found)+1, path, name))
              found.append((path, name, opt))

  # we need the '<' because of the UUID, as there are multiple test cases
  # with the same (path, name).
  # TODO: we should fix up the other script [1-find-slow] to also UUID
  #   tag the programs.
  if len(found) < len(slow_programs):
    raise RuntimeError("[len(found):%s] < [len(slow_programs):%s]" % \
      (len(found), len(slow_programs)))
  ...
  with open(out_path, "w") as of:
    ...
    for (path, name, opt) in found:
      uuid += 1
      bitwidth = slow_programs[(path, name)]
      row = verify_opt_with_timeout(uuid, path, opt, timeout, bitwidth)
